...
export default function PageStadium() {
    ...
    const schemeStatus = useSelector((state) =>
        getStadiumSchemeStatus(state, id)
    );
    ...
    return (
        <Form
            layout='vertical'
            onFinish={(values) => {
                const { name, address, country, city, scheme, scheme_blob } =
                    values;
                const stadium = {
                    ...name,
                ...
                if (scheme)
                    stadium.scheme = JSON.stringify(scheme).replaceAll(
                        '"',
                        "'"
                    );
                if (!isNew) stadium.id = id;
                dispatch(postData({ stadiums: [stadium] })).then(() =>
                    navigate('/stadiums')
                );
            }}
        ...
        >
            ...
            <Row style={{ margin: '20px 20px 0 20px' }}>
                <Col span={24} style={{ padding: '0 10px 0 0' }}>
                    <Form.Item label='Image scheme' name='scheme_blob'>
                        <InputImage />
                    </Form.Item>
                </Col>
            </Row>
            <Row style={{ margin: '20px 20px 0 20px' }}>
                <Col span={12} style={{ padding: '0 10px 0 0' }}>
                    <Form.Item label='Json scheme' name='scheme'>
                        <JSONEditor />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item label='Json scheme preview' name='scheme'>
                        <StadiumScheme />
                    </Form.Item>
                </Col>
            </Row>
